...
foreach($this->Mench_ledger->fetch($filters, array('x__next'), 0) as $expires){
    ...
    foreach($this->Mench_ledger->fetch(array(
        ...
        if(!count($answer_completed) && intval( $expires['x__message'])>0 && $seconds_left <= 0){
            ...
            if($deleted){
                $links_deleted++;
                echo '<div style="padding-left: 21px;">'.$links_deleted.') <a href="'.view__memory(42903,42902).$x_progress['e__handle'].'">'.$x_progress['e__title'].'</a>: '.$x_progress['x__time'].' ? '.$x_progress['x__message'].' / <a href="'.view__app_link(12722).'?x__id=' . $x_progress['x__id'] . '">'.$x_progress['x__id'].' / Answer: '.count($answer_completed).'</a> '.( !count($answer_completed) ? ( $seconds_left <= 0 ? ' DELETE ' : '['.$seconds_left.'] SEcs left' ) : '' ).' ('.intval( $expires['x__message']) .'+'. $buffer_time .'-'. time() .'-'. strtotime($x_progress['x__time'] ).' = '.$seconds_left.')</div>';
            }
